...
public class MyOneToOneServer {
	private static final Logger logger = LoggerFactory.getLogger(MyOneToOneServer.class);
	...
	//临时放些消息，之后直接搞到数据库中
	private static List msg = new LinkedList<Message>();
	...
	public void open(Session session, @PathParam(value = "token") String token) {
		...
		socketServers.add(new Client(token, session));
	...
	public void onMessage(String message) {
		...
		sendMessage(m.getContent(), m.getOwn());
